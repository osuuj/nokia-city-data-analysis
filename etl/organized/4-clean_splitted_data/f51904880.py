...
def filter_english_descriptions(descriptions):
    if not descriptions:
        return []
    return [desc['description'] for desc in descriptions if desc.get('languageCode') == "3"]
...
def process_json_file(file_path, part):
    ...
    for company in data:
        ...
        business_info.append({
            ...
            "main_business_line": "; ".join(filter_english_descriptions(company.get('mainBusinessLine', {}).get('descriptions'))),
        ...
        for address in company.get('addresses', []):
            for post_office in address.get('postOffices', []):
                if post_office.get('languageCode') == "3":
