...
def clean_data(df):
    ...
    # 4. Flatten nested structures (example: 'businessId', 'euId')
    ...
    if 'euId.value' in df_cleaned.columns:
        df_cleaned['euId'] = df_cleaned['euId.value']
        df_cleaned = df_cleaned.drop(columns=['euId.value', 'euId.source'], errors='ignore')
    
    # 5. Normalize data (example: convert dates to a standard format)
