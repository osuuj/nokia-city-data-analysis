...
def save_processed_data(cleaned_dir: str, city: str, idx: int, data: Dict[str, Any], project_dir: str) -> None:
    ...
    for key, df in data.items():
        ...
        logging.info(f"Processed {key} data saved to {os.path.relpath(csv_filename, project_dir)}")
...
def process_data(city: str, project_dir: str, city_paths: dict) -> None:
    ...
    try:
        ...
        if not json_files:
            logging.error(f"No JSON files found in {split_dir}")
            return
        ...
        for idx, json_file in enumerate(json_files):
            ...
            if json_data is None:
                logging.error(f"JSON data is None for file {json_file}")
                continue
            ...
            save_processed_data(cleaned_dir, city, idx, processed_data, project_dir)
            
    except Exception as e:
        logging.error(f"An error occurred while processing data for {city}. Error: {e}")
