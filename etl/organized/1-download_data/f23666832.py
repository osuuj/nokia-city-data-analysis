...
PROCESSED_DIR = 'data/processed/{city}'
READY_FOR_DB_DIR = 'data/ready_for_db'

# Function to generate paths based on city name
def get_city_paths(city: str) -> Tuple[str, str, str, str]:
    city_filtered_path = f'{PROCESSED_DIR.format(city=city)}/filtered/{city}_companies.json'
    split_dir = f'{PROCESSED_DIR.format(city=city)}/split'
    cleaned_dir = f'{PROCESSED_DIR.format(city=city)}/cleaned'
    ready_for_db_dir = READY_FOR_DB_DIR
    return city_filtered_path, split_dir, cleaned_dir, ready_for_db_dir
