...
# Function to filter English descriptions
def filter_english_descriptions(descriptions):
    return [desc['value'] for desc in descriptions if desc['languageCode'] == '3']

# Function to map company form type
def map_company_form_type(form_type):
    # Implement the mapping logic here
    return form_type

# Function to map name type
def map_name_type(name_type):
    # Implement the mapping logic here
    return name_type
...
def save_to_csv(file_path, data, headers):
    # Implement the CSV saving logic here
    pass
...
def process_json_file(file_path, part):
    ...
    for company in data:
        ...
        append_to_table(business_info, {
            ...
            "primary_name": company['names'][0]['name'] if company.get('names') else None,
    ...
    save_to_csv(f'{CLEANED_DIR}/cleaned_{CITY}_part_{part}_addresses.csv', addresses_table, [
        ...
        "building_number", "entrance", "apartment_number", "co", "country", "website"
    ])
