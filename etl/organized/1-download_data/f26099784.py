...
SPLIT_DIR = 'data/processed/{city}/split'
CLEANED_DIR = 'data/processed/{city}/cleaned'
READY_FOR_DB_DIR = 'data/ready_for_db'

# Function to generate paths based on city name
def get_city_paths(city):
    city_filtered_path = f'data/processed/{city}/filtered/{city}_companies.json'
    split_dir = SPLIT_DIR.format(city=city)
    cleaned_dir = CLEANED_DIR.format(city=city)
    return city_filtered_path, split_dir, cleaned_dir
